...
func EncodePubKey(pub *PublicKey) string{
	bytes := elliptic.Marshal(pub.Curve, pub.X,pub.Y)
	return hex.EncodeToString(bytes)
}


func DecodePubKey(base64String string) (*PublicKey, error){
	bytes,err := hex.DecodeString(base64String)
	if err != nil {
		return nil, err
	}
	var curve = P256Sm2 ()
	x, y := elliptic.Unmarshal(curve, bytes)
	pubilcKey :=&PublicKey{curve,x,y}
	return pubilcKey, nil
}

func EncodePrivKey(priv *PrivateKey) string{
	bytes := priv.D.Bytes()
	return hex.EncodeToString(bytes)
}

func DecodePrivKey(base64String string) (*PrivateKey, error){
	c := P256Sm2()
	bytes,err := hex.DecodeString(base64String)
	if err != nil {
		return nil, err
